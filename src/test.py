...
class MainApp:
    def __init__(self):
        ...
        self.doc_path = os.path.normpath(
            os.path.join(_buf.value, "My Games", "Skyrim Special Edition")
        )
        # logging
        self.log_name = (
        ...
        self.log_path = os.path.join(
            os.getenv("APPDATA"), self.name, "logs", self.log_name
        )
        ...
        try:
            ...
            if len(config.keys()) < len(self.default_conf.keys()):
                ...
                print(
                    f"Detected incomplete (outdated) config file. Updating with default config..."
                )
                new_keys = [
                    key for key in self.default_conf.keys() if key not in config.keys()
                ]
                for key in new_keys:
        ...
        self.log_level = getattr(
            logging, self.config["log_level"].upper(), 20
        )  # info level
        ...
        if self.compiled:
            parser = argparse.ArgumentParser(
                prog=os.path.basename(sys.executable), description=help_msg
            )
        else:
            parser = argparse.ArgumentParser(
                prog=f"{os.path.basename(sys.executable)} {__file__}",
        ...
        parser.add_argument(
            ...
            choices=list(main.LOG_LEVELS.values()),
        )
        parser.add_argument("--no-log", help="Disable log file.", action="store_true")
        parser.add_argument(
            "--keep-log",
            help="Keep log file. (Overwrites --no-log)",
            action="store_true",
        )
        ...
        self.log.debug(
            f"{'Detected platform':21}: {platform.system()} {platform.version()} {platform.architecture()[0]}"
        )
    ...
    def migrate(self):
        ...
        if self.destination == "ModOrganizer":
            appdata_path = os.path.join(
                os.getenv("LOCALAPPDATA"), "ModOrganizer", self.dst_modinstance.name
            )
        ...
        if self.source == "Vortex":
            ...
            vortex_path = os.path.join(
                os.getenv("APPDATA"), "Vortex", self.game.lower()
            )
            loot_path = os.path.join(
                os.getenv("LOCALAPPDATA"), "LOOT", "games", self.game_instance.name
            )
            ...
            if os.path.isdir(loot_path):
                ...
                if os.path.isfile(os.path.join(loot_path, "userlist.yaml")):
                    os.rename(
                        os.path.join(loot_path, "userlist.yaml"),
                        os.path.join(loot_path, f"userlist.yaml.mmm_{c}"),
                    )
                    self.log.debug(
                        f"Renamed installed LOOT's 'userlist.yaml' to 'userlist.yaml.mmm_{c}'."
                    )
                shutil.copyfile(
                    os.path.join(vortex_path, "userlist.yaml"),
                    os.path.join(loot_path, "userlist.yaml"),
                )
                self.log.debug(
                    "Copied userlist.yaml from Vortex' LOOT to installed LOOT."
                )
            ...
            if self.src_modinstance.stagefiles:
                if "y" in input("Detected root files. Copy to gamefolder? (y or n)\n>"):
                    ...
                    for c, mod in enumerate(mods_to_copy):
                        shutil.copytree(
                            os.path.join(self.src_modinstance.stagefile.dir, mod),
                            installdir,
                            dirs_exist_ok=True,
                        )
        ...
        for c, mod in enumerate(self.src_modinstance.mods.keys()):
            self.dst_modinstance.import_mod(
                os.path.join(self.src_modinstance.paths["instance_path"], mod)
            )
        ...
        if src_dls := self.src_modinstance.paths.get("download_path", None):
            ...
            if src_dls != self.dst_modinstance.paths["download_path"]:
                try:
                    self.log.debug(f"Mode: {self.mode}")
                    for c, archive in enumerate(
                        os.listdir(self.src_modinstance.paths["download_path"])
                    ):
                        src_path = os.path.join(
                            self.src_modinstance.paths["download_path"]
                        )
                        dst_path = os.path.join(
                            self.dst_modinstance.paths["download_path"]
                        )
                        if self.mode == "copy":
...
def main_test():
    ...
    if app.source == "Vortex":
        ...
        instance_path = os.path.join(
            os.getenv("LOCALAPPDATA"), "ModOrganizer", instance_data["name"]
        )
        instance_data["paths"] = {}
        ...
        with open(ini_file, "r") as file:
            for line in file.readlines():
                if line.startswith("base_directory="):
                    instance_data["paths"]["instance_path"] = os.path.normpath(
                        line.split("=", 1)[1].strip()
                    )
                    instance_path = instance_data["paths"]["instance_path"]
                elif line.startswith("download_directory="):
                    instance_data["paths"]["download_path"] = os.path.normpath(
                        line.split("=", 1)[1]
                    )
        # Get mods from mods folder
        mods = [
            os.path.join(instance_path, "mods", mod)
            for mod in os.listdir(os.path.join(instance_path, "mods"))
            if os.path.isdir(os.path.join(instance_path, "mods", mod))
        ]
    ...
    if app.destination == "Vortex":
        instance_data = {"name": "", "paths": {"staging_folder": ""}}
        ...
        appdata_path = os.path.join(
            os.getenv("LOCALAPPDATA"), "ModOrganizer", instance_name
        )
        if os.path.isdir(appdata_path):
    ...
    if not _exit:
        ...
        app.log.debug(
            f"Testing took {time.time() - starttime} second(s). ({(time.time() - starttime) / 60} minute(s))"
        )
